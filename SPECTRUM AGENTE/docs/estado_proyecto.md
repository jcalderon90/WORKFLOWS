...
- ✅ **Completado (2026-06-01)**: **RSVP: persistencia progresiva de citas — fix crítico:** Diagnóstico confirmado en Mongo (33 docs): 2 leads `llamada` y 1 `cita_presencial` con `created_at === updated_at` y sin `intencion`/`confirmada` — el gate `If TIENE FECHA` solo abría la persistencia cuando había `fecha_hora`, dependiendo de que el LLM re-emitiera todos los campos en cada turno. Si el LLM omitía `fecha_hora` en el turno de confirmación, la cita quedaba truncada y la notificación al asesor nunca se disparaba. Fix: (1) gate ampliado para abrir con cualquier dato de cita (`tipo_agendamiento` OR `fecha_hora` OR `horario_preferido` OR `cita_confirmada`), creando el appointment al elegir tipo y actualizando en cada turno; (2) `IF APPOINTMENT EXIST` migrado de `.item` → `.first()`; (3) `Update Appointment` añade `dateFields: updated_at` para tipo Date uniforme; (4) prompt reforzado con regla 🔒 anti-null (nunca emitir `null` en campo ya capturado). Deploy: `TjFPzHs5aimxILH7`. Commit: `464106c`.
...
- ✅ **Completado (2026-06-01)**: **Guard tipo+fecha en `_FechaCita` y `_TipoCita`:** Con la persistencia progresiva de RSVP, los appointments ahora se crean desde el turno en que el lead elige el tipo (antes de tener fecha). Para evitar que Sync_CRM enviara `_TipoCita` al CRM con citas parciales o abandonadas, se añadió un guard: ambos campos (`_FechaCita` y `_TipoCita`) solo se emiten cuando el appointment tiene **tanto `tipo` como `fecha_hora`**. El fallback a `CRM_Data` (valores de sync anterior válida) sigue activo. Deploy: `TTVNRX38pPoPmK2X`. Commit: `464106c`.
...
- **RSVP: persistencia progresiva (2026-06-01):** Fix crítico — citas parciales y abandonadas ya no quedan sin `intencion`/`confirmada`. El appointment se crea al elegir tipo y se actualiza en cada turno. Diagnóstico con datos reales de Mongo confirmó el problema. Deploy ambos workflows (`TjFPzHs5aimxILH7`, `TTVNRX38pPoPmK2X`). Commit `464106c`.
- **Test E2E mejorado (2026-06-01):** Escenario `rsvp_flow` en `test_agent.py` reescrito — 8 turnos que pasan el gate name+email+phone y recorren el flujo completo (tipo→fecha→habitaciones→intención→confirmar). Fix SSL para macOS (certifi + fallback `CERT_NONE`). La validación del fix en BD se hace vía MCP Mongo post-ejecución.
