...
> Última actualización: 2026-05-27 (Sync_CRM: Protección de atribución Fase 2 completada · Lead Collector: _Comentarios enviados solo aquí · Campos opcionales validados según tabla SOAP oficial)
...
- ✅ **Completado (2026-05-27)**: **Protección de atribución para leads Fase 2:** Implementada lógica condicional en nodo `Body` de `Sync_CRM.json` para omitir campos de atribución que Tribal ya tiene cuando `fase_2: true`. Campos omitidos: `_OrigenCliente`, `_UTMSource`, `_UTMCampaing`, `_MetodocontactoPref`. Campos que SÍ se envían para Fase 2 (si existen): datos de actualización (`_NumeroHabitaciones`, `_EstadoCivil`, `_MotivoInteres`, `_FechaCita`, `_TipoCita`, `_ResumenConversacion`, `_DudasCliente`) y opcionales (`_CorreoSecundario`). Evita sobrescritura de datos pre-cargados por Tribal en Dynamics 365. Deploy: 2026-05-27.
